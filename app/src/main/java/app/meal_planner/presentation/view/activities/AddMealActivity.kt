...
class AddMealActivity : AppCompatActivity(R.layout.activity_add_meal) {
        ...
        add_meal.setOnClickListener {
            ...
            if(title.isNotEmpty() && calorieCount() > 0){
                ...
                when(calledFrom){
                    ...
                    "Today_Edit" -> mealsViewModel.updateMealAndItems(MealWithItemsEntity(MealEntity(meal!!.meal.id, title.toString(), calorieCount(), carbsCount(), proteinCount(), fatCount(), true, existing), list))
                    "Existing_Add" -> mealsViewModel.insertMeal(MealWithItemsEntity(MealEntity(0, title.toString(), calorieCount(), carbsCount(), proteinCount(), fatCount(), today = false, existing = true), list))
                    "Existing_Edit" -> mealsViewModel.updateMealAndItems(MealWithItemsEntity(MealEntity(meal!!.meal.id, title.toString(), calorieCount(), carbsCount(), proteinCount(), fatCount(), meal!!.meal.today, true), list))
                }
                finish()
            }else{
                if(title.isEmpty()){
            ...
            if(it.ingredient_title != null && it.ingredient_carbs != null && it.ingredient_protein != null && it.ingredient_fat != null){
                ...
                if(titleText == ""){
                    ...
                    if (meal == null){
                        ...
                        mutableList.add(ItemEntity(0, meal!!.meal.id, titleText,carbs, protein, fat))
                    }
    ...
    private fun populateFields(mealWithItems: MealWithItems) {
        ...
        checkbox.isChecked = mealWithItems.meal.existing
        ...
        mealWithItems.items.forEach {
            ...
            kcalNew += (it.carbs * 4) + (it.protein * 4) + (it.fat * 9)
            carbsNew += it.carbs
            proteinNew += it.protein
            fatNew += it.fat
        }
        calories.text = kcalNew.toString() + "kcal"
        carbs.text = carbsNew.toString() + "g"
    ...
    @SuppressLint("SetTextI18n")
    private fun addListenersToView(rowView: View) {
